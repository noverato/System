...
function initEvoHallDisplay() {
    ...
    if (canChooseClass) {
        ...
        Object.keys(map).forEach(id => {
            ...
            if (el) {
                el.setAttribute('data-action', 'chooseClass');
                el.setAttribute('data-args', JSON.stringify([map[id]]));
            }
        });
    }
        ...
        if (side) {
            side.innerHTML = `
                <div style="border-top:1px solid var(--gold); padding-top:10px;">
                    ...
                    <button class="btn-action" style="width:100%; margin-bottom:8px;" data-action="choosePath" data-args='["Licht"]'>☀️ LICHT</button>
                    <button class="btn-action" style="width:100%; background:#333;" data-action="choosePath" data-args='["Dunkel"]'>🌑 DUNKEL</button>
                </div>
            `;
        }
    }
    ...
    if (side && tier === 0) {
        side.innerHTML = `
            <div style="border-top:1px solid var(--gold); padding-top:10px;">
                ...
                <button class="btn-action" style="width:100%; margin-top:8px;" ${canSprout ? '' : 'disabled'} data-action="evolveTo" data-args='[1]'>🌱 FREISCHALTEN</button>
            </div>
        `;
    }
}
