    ...
    function updateAvatarWeapons() {
        ...
        if (data.equipment.weapon) {
            ...
            if (weaponSprite) {
                ...
                // Position relativ zum Avatar
                weaponSprite.position.set(weaponOffsetX, weaponOffsetY, weaponOffsetZ);
            }
        }
        if (data.equipment.offhand) {
            offhandSprite = createWepSprite(data.equipment.offhand, true);
            if (offhandSprite) {
                offhandSprite.rotation.y = -Math.PI / 2;
                offhandSprite.rotation.z = -(weaponRotDeg * Math.PI) / 180;
                
                avatar.add(offhandSprite);
                offhandSprite.position.set(-weaponOffsetX, weaponOffsetY, weaponOffsetZ);
            }
        }
    }
